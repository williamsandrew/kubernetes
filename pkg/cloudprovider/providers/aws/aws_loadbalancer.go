...
func (s *AWSCloud) ensureLoadBalancer(namespacedName types.NamespacedName, name string, listeners []*elb.Listener, subnetIDs []string, securityGroupIDs []string, internalELB, proxyProtocol bool) (*elb.LoadBalancerDescription, error) {
	...
	if loadBalancer == nil {
		...
		if proxyProtocol {
			err = s.createProxyProtocolPolicy(name)
			if err != nil {
				return nil, err
			}
			...
			for _, listener := range listeners {
				...
				err := s.setBackendPolicies(name, *listener.InstancePort, []*string{aws.String(ProxyProtocolPolicyName)})
				if err != nil {
					return nil, err
				}
			}
...

		{
			...
			for _, listener := range listeners {
				...
				if setPolicy {
					...
					err := s.setBackendPolicies(name, instancePort, proxyPolicies)
					if err != nil {
						return nil, err
					}
					dirty = true
			...
			for instancePort, found := range foundBackends {
				if !found {
					...
					err := s.setBackendPolicies(name, instancePort, []*string{})
					if err != nil {
						return nil, err
					}
					dirty = true
